...
const REG8H: [&str; 8] = ["al", "cl", "dl", "bl", "ah", "ch", "dh", "bh"];
...
const JCC: [&str; 16] = [
    "jo", "jno", "jb", "jnb", "jz", "jnz", "jbe", "ja", "js", "jns", "jp", "jnp", "jl", "jnl",
    "jle", "jg",
];
const CMOVCC: [&str; 16] = [
...
const SYSOP: [&str; 8] = ["sgdt", "sidt", "lgdt", "lidt", "smsw", "?", "lmsw", "?"];

fn reg64(rm: u8, ext: u8) -> &'static str {
    REG64[(rm | (ext << 3)) as usize]
}
fn reg32(rm: u8, ext: u8) -> &'static str {
    REG32[(rm | (ext << 3)) as usize]
}
fn reg16(rm: u8, ext: u8) -> &'static str {
    REG16[(rm | (ext << 3)) as usize]
}
fn reg8(rm: u8, rex: bool, ext: u8) -> &'static str {
    if rex {
        REG8L[(rm | (ext << 3)) as usize]
    } else {
        REG8H[(rm | (ext << 3)) as usize]
    }
}
...
pub fn disasm_one(addr: u64, bytes: &[u8], w: &mut impl Write) -> Option<usize> {
    ...
    match op {
        ...
        | b @ 0x38..=0x3B => {
            ...
            if let Some(c) = modrm_rm(
                w, rest, to_reg, byte_sz, wide, has_rex, ext_r, ext_b, ext_x, addrsz32, opsz16,
            ) {
                pos += c;
            }
        }
        ...
        // -- ALU AL/EAX, imm8 --
        b @ 0x04 | b @ 0x0C | b @ 0x14 | b @ 0x1C | b @ 0x24 | b @ 0x2C | b @ 0x34 | b @ 0x3C => {
            let grp = ((b >> 3) & 7) as usize;
        ...
        // -- ALU EAX/RAX, imm32 --
        b @ 0x05 | b @ 0x0D | b @ 0x15 | b @ 0x1D | b @ 0x25 | b @ 0x2D | b @ 0x35 | b @ 0x3D => {
            let grp = ((b >> 3) & 7) as usize;
            if wide {
                let _ = write!(w, "{} rax,", GRP1[grp]);
            } else {
                let _ = write!(w, "{} eax,", GRP1[grp]);
            }
            if pos + 4 <= bytes.len() {
        ...
        | 0x61 | 0x62 | 0x9A | 0xC4 | 0xC5 | 0xCE | 0xD4 | 0xD5 | 0xD6 | 0xEA => {
            let _ = write!(w, "db {:#x}", op);
        }

        // -- PUSH reg / POP reg --
        0x50..=0x57 => {
            let _ = write!(w, "push {}", reg64(op - 0x50, ext_b));
        }
        0x58..=0x5F => {
            let _ = write!(w, "pop {}", reg64(op - 0x58, ext_b));
        }
        ...
        0x63 => {
            ...
            if let Some(c) = write_ea(
                w, rest, addrsz32, ext_b, ext_x, byte_sz, wide, has_rex, opsz16,
            ) {
                pos += c;
            }
        }
        ...
        0x91..=0x97 => {
            let r = op - 0x91;
            if wide {
                let _ = write!(w, "xchg rax,{}", reg64(r, ext_b));
            } else {
                let _ = write!(w, "xchg eax,{}", reg32(r, ext_b));
            }
        }
        ...
        0xA1 => {
            ...
            if pos + moff_sz <= bytes.len() {
                ...
                pos += moff_sz;
                if wide {
                    let _ = write!(w, "mov rax,[{:#x}]", v);
                } else {
                    let _ = write!(w, "mov eax,[{:#x}]", v);
                }
            }
        }
        0xA2 => {
        ...
        0xA3 => {
            ...
            if pos + moff_sz <= bytes.len() {
                ...
                pos += moff_sz;
                if wide {
                    let _ = write!(w, "mov [{:#x}],rax", v);
                } else {
                    let _ = write!(w, "mov [{:#x}],eax", v);
                }
            }
        }
        ...
        0xA9 => {
            if pos + 4 <= bytes.len() {
                ...
                pos += 4;
                if wide {
                    let _ = write!(w, "test rax,{:#x}", v);
                } else {
                    let _ = write!(w, "test eax,{:#x}", v);
                }
            }
        }
        ...
        0xE3 => {
            if pos < bytes.len() {
                ...
                let t = addr.wrapping_add(pos as u64).wrapping_add(off as u64);
                if addrsz32 {
                    let _ = write!(w, "jecxz {:#x}", t);
                } else {
                    let _ = write!(w, "jrcxz {:#x}", t);
                }
            }
        }
        ...
        0xE7 => {
            if pos < bytes.len() {
                let v = bytes[pos];
                pos += 1;
                let _ = write!(w, "out {:#x},eax", v);
            }
        }
        0xEC => {
            let _ = write!(w, "in al,dx");
        }
        0xED => {
            let _ = write!(w, "in eax,dx");
        }
        0xEE => {
            let _ = write!(w, "out dx,al");
        }
        0xEF => {
            let _ = write!(w, "out dx,eax");
        }
        ...
        0xFF => {
            ...
            match grp {
                ...
                3 => {
                    let _ = write!(w, "call far ");
                }
                4 => {
                    let _ = write!(w, "jmp ");
                }
                5 => {
                    let _ = write!(w, "jmp far ");
                }
                6 => {
                    let _ = write!(w, "push ");
                }
        ...
        0x0F => {
            ...
            match ext {
                ...
                0x01 => {
                    ...
                    if (modrm >> 6) == 3 {
                        ...
                        match (reg, modrm & 7) {
                            ...
                            (1, _) => {
                                let _ = write!(w, "vmmcall");
                                pos += 1;
                            }
                            (2, _) => {
                                let _ = write!(w, "vmload");
                                pos += 1;
                            }
                            (3, _) => {
                                let _ = write!(w, "vmsave");
                                pos += 1;
                            }
                            (4, _) => {
                                let _ = write!(w, "smsw");
                            ...
                            (7, 0) => {
                                let _ = write!(w, "swapgs");
                                pos += 1;
                            }
                            (7, 1) => {
                                let _ = write!(w, "rdtscp");
                                pos += 1;
                            }
                            _ => {
                                let _ = write!(w, "sysop {:#x},{:#x}", reg, modrm & 7);
                                pos += 1;
                            }
                        }
                    } else {
                ...
                0x05 => {
                    let _ = write!(w, "syscall");
                }
                0x06 => {
                    let _ = write!(w, "clts");
                }
                0x07 => {
                    let _ = write!(w, "sysret");
                }
                0x08 => {
                    let _ = write!(w, "invd");
                }
                0x09 => {
                    let _ = write!(w, "wbinvd");
                }
                0x0B => {
                    let _ = write!(w, "ud2");
                }
                0x0D => {
                    let _ = write!(w, "prefetch");
                    if let Some(c) = write_ea(
                        w, rest, addrsz32, ext_b, ext_x, byte_sz, wide, has_rex, opsz16,
                    ) {
                        pos += c;
                ...
                // -- SYSENTER / SYSEXIT --
                0x34 => {
                    let _ = write!(w, "sysenter");
                }
                0x35 => {
                    let _ = write!(w, "sysexit");
                }
                ...
                // -- PUSH/POP FS/GS --
                0xA0 => {
                    let _ = write!(w, "push fs");
                }
                0xA1 => {
                    let _ = write!(w, "pop fs");
                }
                0xA8 => {
                    let _ = write!(w, "push gs");
                }
                0xA9 => {
                    let _ = write!(w, "pop gs");
                }
                0xA2 => {
                    let _ = write!(w, "cpuid");
                }
                ...
                0xB6 => {
                    ...
                    let _ = write!(w, ",");
                    if let Some(c) =
                        write_ea(w, rest, addrsz32, ext_b, ext_x, true, wide, has_rex, opsz16)
                    {
                        pos += c;
                    }
                }
                0xB7 => {
                    ...
                    let _ = write!(w, ",");
                    if let Some(c) =
                        write_ea(w, rest, addrsz32, ext_b, ext_x, false, wide, has_rex, true)
                    {
                        pos += c;
                    }
                }
