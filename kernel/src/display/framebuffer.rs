//! Framebuffer text renderer with built-in 8x16 bitmap font.
//!
//! # Invariants
//! - INV-FB-01: ptr is non-null and 4-byte aligned
//! - INV-FB-02: ptr is valid for stride * height * 4 bytes
//! - INV-FB-03: width <= stride (stride = pixels per scanline; bytes/row = stride * 4)
//! - INV-FB-04: draw_char() never writes outside valid region
//! - INV-FONT-01: FONT has exactly 128 entries (ASCII 0-127)
//! - INV-FONT-02: Each entry is exactly 16 bytes (8x16 bitmap)
//! - INV-FONT-03: FONT is stored in .rodata (never mutated)
...
impl Framebuffer {
    ...
    ///
    /// # Safety
    /// - addr must be valid for width * height * 4 bytes
    /// - addr must be properly aligned (4-byte alignment)
    ...
    pub unsafe fn new(addr: u64, width: usize, height: usize, stride: usize, pixel_format: PixelFormat) -> Self {
        ...
        // INV-FB-03: stride is pixels-per-scanline, so width <= stride.
    ...
    /// Get the raw pointer to the framebuffer.
    ...
    /// Get framebuffer stride in pixels (bytes per row = stride * 4).
    pub fn stride(&self) -> usize {
        self.stride
    }

    /// Get the pixel format of the framebuffer.
    pub fn pixel_format(&self) -> crate::boot::PixelFormat {
        self.pixel_format
    }
}
...
impl Display for Framebuffer {
    ...
    fn draw_char(&mut self, x: usize, y: usize, ch: u8) -> bool {
        ...
        // INV-FONT-01: ch < 128, so index is valid
        ...
        for row in 0..16 {
            ...
            for col in 0..8 {
                ...
                // INV-FONT-02: glyph[row] is valid (16 bytes per char)
                let offset = py * self.stride * 4 + px * 4;
                // INV-FB-04: offset < stride * height * 4
                unsafe {
                    if glyph[row] & (1 << (7 - col)) != 0 {
                        match self.pixel_format {
                            PixelFormat::Bgr => {
                                *self.ptr.add(offset) = 0xFF;     // Blue
                                *self.ptr.add(offset + 1) = 0xFF; // Green
                                *self.ptr.add(offset + 2) = 0xFF; // Red
                                *self.ptr.add(offset + 3) = 0x00; // Reserved
                            }
                            PixelFormat::Rgb => {
                                *self.ptr.add(offset) = 0xFF;     // Red
                                *self.ptr.add(offset + 1) = 0xFF; // Green
                                *self.ptr.add(offset + 2) = 0xFF; // Blue
                                *self.ptr.add(offset + 3) = 0x00; // Reserved
                            }
                        }
                    } else {
                        // Clear background pixel to black.
                        *self.ptr.add(offset) = 0x00;
                        *self.ptr.add(offset + 1) = 0x00;
                        *self.ptr.add(offset + 2) = 0x00;
                        *self.ptr.add(offset + 3) = 0x00;
                    }
                }
            }
        }
...
// INV-FONT-03: FONT is in .rodata, 128 * 16 = 2048 bytes
// Standard 8x16 bitmap font for ASCII characters 0-127
// Each character is 16 bytes, each byte is a row of 8 pixels
// Bit 7 = leftmost pixel, Bit 0 = rightmost pixel
pub(crate) static FONT: [[u8; 16]; 128] = [
    // ASCII 0-31: Control characters (displayed as blanks or simple shapes)
    ...
    // ASCII 32-127: Standard printable characters
