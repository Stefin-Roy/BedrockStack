...
pub(crate) fn port_out(port: u16, value: u32, width: u8) -> Result<(), AcpiError> {
    unsafe {
        match width {
            8 => {
                core::arch::asm!("out dx, al", in("dx") port, in("al") value as u8, options(nomem, nostack, preserves_flags))
            }
            16 => {
                core::arch::asm!("out dx, ax", in("dx") port, in("ax") value as u16, options(nomem, nostack, preserves_flags))
            }
            32 => {
...
pub fn gas_read(gas: &Gas) -> Result<u64, AcpiError> {
    let width = if gas.register_bit_width > 0 {
        gas.register_bit_width
    } else {
        16
    };
