...
/// Safely writes to a Model Specific Register (MSR).
#[inline]
...
/// Safely executes the CPUID instruction.
///
/// This bypasses LLVM's restriction on using `ebx`/`rbx` directly by transferring
/// the output of `ebx` to an arbitrary compiler-allocated register (`{ebx_out}`)
/// inside the assembly sequence, preserving the host environment safely.
#[inline]
pub unsafe fn cpuid(leaf: u32, subleaf: u32) -> (u32, u32, u32, u32) {
    ...
    let edx: u32;
    asm!(
        // On 64-bit systems, we preserve the full rbx, move CPUID's result to our target register,
        // and restore the original rbx. This is completely safe even in PIC mode.
        "push rbx",
        "cpuid",
        "mov {ebx_out:e}, ebx",
        "pop rbx",
...
/// Throttles the Intel CPU into its absolute slowest operating mode.
...
pub unsafe fn enable_cpu_slow_mode() {
    ...
    // Bit 4 must be 1 to enable modulation.
    // Bits 3:1 = 001 (12.5% duty cycle).
    // Bit 0 MUST be 0 (Reserved). 
    // 0x12 (10010b) is the correct value.
    ...
    if (ecx1 >> 7) & 1 == 1 {
        // High bits set target ratio to the minimum supported P-state (0xFF).
