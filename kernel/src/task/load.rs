...
fn dump_boot_log() {
    ...
    // Path-based helper: TRUNC first chunk, then positioned writes; whole-file retry on IOMMU.
    let write_all = |use_trunc: bool| -> bool {
        use crate::filesystems::vfs::types::OpenFlags;
        let mut off = 0usize;
        let mut written = 0usize;
        while off < bytes.len() {
            let chunk = &bytes[off..core::cmp::min(off + 4096, bytes.len())];
            let res = if use_trunc && off == 0 {
                crate::filesystems::vfs::pwrite(path, 0, chunk, OpenFlags::CREATE | OpenFlags::TRUNC)
            } else {
                crate::filesystems::vfs::pwrite(path, off as u64, chunk, OpenFlags::WRITE)
            };
            match res {
                Ok(n) if n > 0 => {
                    off += n;
    ...
    // First attempt with TRUNC
    if write_all(true) {
        #[cfg(target_arch = "x86_64")]
        crate::bootlog::mark_initial_flushed(bytes.len());
        return;
    }
    SerialPort::puts("[sched] boot.log incomplete, retrying\n");
    ...
    if res.is_ok() {
        ...
        SerialPort::puts("[sched] boot.log fallback via unispace failed\n");
    }
}
