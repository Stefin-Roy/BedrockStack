...
pub fn sfn_from_name(
    name: &str,
    existing_sfns: &HashSet<[u8; MAX_SFN_LEN]>,
) -> Option<[u8; MAX_SFN_LEN]> {
    if name.is_empty() {
        return None;
    }
...
pub fn set_vfat_char(entry: &mut [u8; DIR_ENTRY_SIZE], index: usize, c: u16) {
    ...
    match index {
        0..=4 => {
            entry[1 + index * 2] = bytes[0];
            entry[2 + index * 2] = bytes[1];
        }
        5..=10 => {
            entry[14 + (index - 5) * 2] = bytes[0];
            entry[15 + (index - 5) * 2] = bytes[1];
        }
        11..=12 => {
            entry[28 + (index - 11) * 2] = bytes[0];
            entry[29 + (index - 11) * 2] = bytes[1];
        }
        _ => {}
    }
}
...
pub fn encode_vfat_entries(name: &str, checksum: u8) -> Vec<[u8; DIR_ENTRY_SIZE]> {
    ...
    for i in 0..needed {
        ...
        entry[13] = checksum;
        for j in 0..count {
            set_vfat_char(&mut entry, j, u16_chars[start + j]);
        }
        for j in count..13 {
            set_vfat_char(&mut entry, j, 0xFFFF);
        }
        entries.push(entry);
    }
    entries.reverse();
...
pub fn set_first_clus_in_entry(entry: &mut [u8; DIR_ENTRY_SIZE], cluster: u32) {
    ...
    entry[0x14] = hi_bytes[0];
    entry[0x15] = hi_bytes[1];
    entry[0x1A] = lo_bytes[0];
    entry[0x1B] = lo_bytes[1];
}
