...
pub fn take_to_end_of_pkglength<'a, 'c>(length: PkgLength) -> impl Parser<'a, 'c, Vec<u8>>
where
    'c: 'a,
{
    move |input: &'a [u8], context| {
        ...
        match take_n(bytes_to_take).parse(input, context) {
            Ok((new_input, context, bytes)) => Ok((new_input, context, bytes.to_vec())),
            Err(error) => Err(error),
        }
    }
}
        ...
        loop {
            ...
            match parser.parse(input, context) {
                Ok((new_input, new_context, _)) => {
                    if new_input.len() == len_before {
                        return Err((input, new_context, Propagate::Err(AmlError::NoProgress)));
                    }
                    input = new_input;
                    context = new_context;
                    num_passed += 1;
...
pub fn extract<'a, 'c, P, R>(parser: P) -> impl Parser<'a, 'c, (R, Vec<u8>)>
where
    'c: 'a,
    P: Parser<'a, 'c, R>,
{
    move |input, context: &'c mut AmlContext| {
        ...
        // Do not let parser combinators retain a borrowed view into the AML
        // table.  Some extracted streams are reparsed after the surrounding
        // parser has performed allocations and context changes; keeping them
        // owned makes that lifetime explicit and prevents a stale fat pointer
        // from being carried through a heap allocation.
        let parsed = before[..bytes_parsed].to_vec();
