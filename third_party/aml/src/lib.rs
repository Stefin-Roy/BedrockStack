...
pub enum AmlError {
    ...
    MalformedStream,
    /// Produced when a list-parsing loop (e.g. `TermList`, `FieldList`, or
    /// `PackageElementList`) makes no forward progress through the stream. This
    /// is a BedrockOS hardening addition: without it a parser that accepts a
    /// zero-length token would spin forever.
    NoProgress,
    /// Produced when a `DefWhile` loop exceeds its iteration budget. This is a
    /// BedrockOS hardening addition to bound firmware loops that never make
    /// their predicate false.
    LoopLimitExceeded,
    InvalidNameSeg,
    InvalidPkgLength,
    InvalidFieldFlags,
