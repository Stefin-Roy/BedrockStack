...
/// BedrockOS hardening: cap `DefWhile` iterations so a firmware loop whose
/// predicate never becomes false cannot hang the machine.
            ...
            pkg_length()
                ...
                .then(choice!(
                    opcode(opcode::DEF_ELSE_OP)
                        ...
                        .map(|((), else_branch): ((), Vec<u8>)| Ok(else_branch)),
                    // TODO: can this be `id().map(&[])`?
                    |input, context| -> ParseResult<'a, 'c, Vec<u8>> {
                        /*
        ...
        .then(comment_scope(
            ...
            pkg_length()
                ...
                .feed(move |(length, (first_predicate, predicate_stream))| {
                    take_to_end_of_pkglength(length)
                        .map(move |body| Ok((first_predicate.clone(), predicate_stream.clone(), body)))
                })
                .map_with_context(|(first_predicate, predicate_stream, body), mut context| {
                    ...
                    loop {
                        ...
                        if iterations > MAX_WHILE_ITERATIONS {
                            return (Err(Propagate::Err(AmlError::LoopLimitExceeded)), context);
                        }
