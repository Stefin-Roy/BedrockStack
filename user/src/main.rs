...
//!   0  read(path, buf, buf_len)   — `path` is a NUL-terminated C string.
//!   1  write(path, buf, buf_len)  — same registers; the buffer is an in-place
//!                                   request/response area. The input is
//!                                   decoded first, then the provider's output
//!                                   (or error detail) is rewritten into it
//!                                   from byte 0, zero-filled past it. rax =
//!                                   number of *output* bytes (0 for a plain
//!                                   value write), `< 0` = -errno.
...
/// Invoke a kernel syscall. rdi/rsi/rdx only (r10 is unused in the new ABI).
/// `syscall` clobbers both `rcx` and `r11`, so they are declared as dummy
/// outputs; omitting them lets the compiler keep live values in those
/// registers and return garbage. The result is signed: errors come back as
/// negative errnos, which is the whole point of `isize` here.
#[inline]
...
fn say(dev: &[u8], msg: &[u8]) -> Result<(), isize> {
    ...
    let r = unsafe { syscall(SYS_WRITE, dev.as_ptr() as usize, wbuf.as_mut_ptr() as usize, n) };
    if r >= 0 {
        Ok(())
    } else {
        Err(r)
...
extern "C" fn exit_process(code: usize) -> ! {
    ...
    unsafe {
        syscall(SYS_WRITE, path.as_ptr() as usize, payload.as_mut_ptr() as usize, payload.len());
    }
    loop {}
}
...
pub extern "C" fn entry_main() -> usize {
    ...
    let wr = unsafe { syscall(SYS_WRITE, PATH.as_ptr() as usize, wbuf.as_mut_ptr() as usize, msg_len) };
    if wr < 0 {
    ...
    let mut buf = [0u8; 64];
    let rd = unsafe { syscall(SYS_READ, PATH.as_ptr() as usize, buf.as_mut_ptr() as usize, buf.len()) };
    if rd < 0 {
