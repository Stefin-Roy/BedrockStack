...
- Location: `kernel/src/lib.rs:188-195`
...
`Arch::wake_aps()` for arch-specific startup (IPIs / SBI ecalls).
- Location: `kernel/src/smp/mod.rs:160-164,183-206`

**SMP-010 — `current_cpu_id()` returns the current CPU's ID:**
Convenience wrapper around `current_per_cpu().cpu_id`.
- Location: `kernel/src/smp/mod.rs:108-110`

**SMP-011 — `set_bsp_hardware_id(id)` fills BSP's APIC/hart ID:**
Called after CPU discovery to record the BSP's actual hardware ID
in the PerCpu slot. AP hardware IDs are set during `smp::init()`.
- Location: `kernel/src/smp/mod.rs:144-146`
...
`wake_aps()` which issues IPIs/SBI calls. Collects APs in a `Vec<ApContext>`
(requires heap).
- Location: `kernel/src/smp/mod.rs:172-228`
...
Discovers CPUs via `Arch::discover_cpus()`, allocates AP stacks,
configures PerCpu slots for each AP, builds `Vec<ApContext>`, calls
`Arch::wake_aps()`, returns total CPU count (`u32`).
...
Sets `PER_CPU_SLOTS[0].apic_id` to the BSP's hardware ID. Called after
CPU discovery.
